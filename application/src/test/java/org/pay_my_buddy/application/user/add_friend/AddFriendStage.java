package org.pay_my_buddy.application.user.add_friend;
...
public class AddFriendStage extends Stage<AddFriendStage> {
    ...
    public AddFriendStage a_valid_user() {
        ...
        Mockito.when(userSpi.existsById(user.getId())).thenReturn(true);
        Mockito.when(userSpi.findUser(user.getId())).thenReturn(Optional.of(user));
        return self();
    }

    public AddFriendStage a_valid_friend() {
        friend = UserFaker.create();
        Mockito.when(userSpi.existsById(friend.getId())).thenReturn(true);
        Mockito.when(userSpi.findUser(friend.getId())).thenReturn(Optional.of(friend));
        return self();
    }

    public AddFriendStage an_AddFriendCommand_is_created() {
        try {
            command = new AddFriendCommand(user.getId(), friend.getId());
        } catch (Exception e) {
            commandException = e;
        }
