...
public final class AggregateEntity<AGGREGATE extends AggregateRoot<ID>, ID extends EntityId> {
	...
	public AggregateEntity(AGGREGATE aggregateRoot) {
		...
		this.aggregateData = aggegateToString(aggregateRoot);
	...
	public AGGREGATE aggregate() {
		try {
			var aggregateClass = Class.forName(aggregateType);
			return (AGGREGATE) MAPPER.readValue(aggregateData, aggregateClass);
		} catch (Exception e) {
			throw new InternalErrorException(e.getMessage(), e);
		}
	}

	private String aggegateToString(AGGREGATE aggregate) {
		try {
